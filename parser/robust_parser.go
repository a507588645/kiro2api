...
func (rp *RobustEventStreamParser) parseSingleMessageWithValidation(data []byte) (*EventStreamMessage, int, error) {
	...
	// *** 修复：放宽数据完整性验证，支持流式处理 ***
	// 在流式处理中，数据可能跨越多个块，不应该严格要求长度完全匹配
	if len(data) < int(totalLength) {
		return nil, 0, NewParseError(fmt.Sprintf("数据不完整: 期望 %d 字节，实际 %d 字节", totalLength, len(data)), nil)
	}
...
func (rp *RobustEventStreamParser) parseStreamWithBuffer(data []byte) ([]*EventStreamMessage, error) {
	...
	for {
		...
		if totalLength < config.EventStreamMinMessageSize || totalLength > config.EventStreamMaxMessageSize {
			// *** 修复：改进错误恢复策略，避免数据丢失 ***
			// 尝试寻找下一个可能的消息边界，而不是简单丢弃1字节
			found := false
			for i := 1; i < min(available, 1024); i++ {
				if available >= i+4 {
					nextLength := binary.BigEndian.Uint32(bufferBytes[i : i+4])
					if nextLength >= config.EventStreamMinMessageSize && nextLength <= config.EventStreamMaxMessageSize {
						// 找到可能的下一个消息，跳过到这里
						rp.buffer.Next(i)
						found = true
						logger.Debug("找到下一个可能的消息边界",
							logger.Int("skipped_bytes", i),
							logger.Int("next_length", int(nextLength)))
						break
					}
				}
			}
