...
func (h *ThinkingEventHandler) Handle(message *EventStreamMessage) ([]SSEEvent, error) {
	...
	// 兼容多种字段名：优先使用 content 字段，其次使用 thinking 字段
	// 这是因为上游可能返回不同的字段名
	content, _ := data["content"].(string)
	if content == "" {
		// 尝试使用 thinking 字段
		content, _ = data["thinking"].(string)
	}
	if content == "" {
		logger.Debug("thinking 事件内容为空，跳过处理",
	...
	logger.Debug("处理 thinkingEvent",
		logger.Int("content_length", len(content)),
		logger.String("source_field", getThinkingSourceField(data)))
...
// getThinkingSourceField 获取thinking内容的来源字段名（用于调试日志）
func getThinkingSourceField(data map[string]any) string {
	if _, ok := data["content"].(string); ok {
		return "content"
	}
	if _, ok := data["thinking"].(string); ok {
		return "thinking"
	}
	return "unknown"
}
